...
def get_model(with_downstream_arch=True):
    ...
    if with_downstream_arch:
        from downstream_architectures import STTDownstreamArch
        ...
        downstream_architecture = STTDownstreamArch(
            visual_embedder,
            stt_checkpoint=project_definitions.CHECKPOINT_PATH_STT,
            seg_mask_threshold=0.3,
            mask_cleaning_kernel_size=3,
            use_all_fine_glimpse_locs=False,    # setting to True increases performance applying STT on each fine glimpse loc, but sacrifices efficiency
            crop_size=(700, 700),
            device=DEVICE,
        )
    else:
